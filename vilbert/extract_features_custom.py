...
class FeatureExtractor:
    ...
    def __init__(self, model_file, config_file, batch_size=2, image_dir="", output_folder=""):
        
        list_args = []
        list_args.append("--model_file")
        list_args.append(model_file)
        
        list_args.append("--config_file")
        list_args.append(config_file)
        
        list_args.append("--batch_size")
        list_args.append(str(batch_size))
        
        list_args.append("--image_dir")
        list_args.append(image_dir)
        
        list_args.append("--output_folder")
        list_args.append(output_folder)        
        
        self.args = self.get_parser().parse_args(list_args)
    ...
    """
    input: 
        list_images: list of image_files
    output:
        list of features (num_images, num_featrues(100), dim(2048))
    """
    def extract_features_direct(self, list_image_files):
...
#                     list_features.append(features[idx].cpu().numpy())
#                     list_infos.append(infos[idx])

                    file_base_name = os.path.basename(file_name)
                    file_base_name = file_base_name.split(".")[0]
                    infos[idx]["image_id"] = file_base_name
                    infos[idx]["features"] = features[idx].cpu().numpy()
                    list_infos.append(infos[idx])
